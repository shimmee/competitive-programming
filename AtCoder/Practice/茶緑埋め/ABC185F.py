...
# ---------- Ideas ----------
# セグ木のupdateとクエリ取得をそのままやる
# updateが，ただの置き換えではなく，これまでの値 xor 新しい値，なのでこの点に注意
...
seg = SegTree(A, operator=xor, ide_ele=False)
for _ in range(q):
    t, x, y = map(int, input().split())
    if t == 1: # 値の更新
        seg.update(x-1, y)
    else:
        print(seg.get_query(x-1, y))
